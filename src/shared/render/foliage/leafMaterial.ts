...
import type { LeafSurface } from './types.ts';
...
export class LeafNodeMaterial extends THREE.MeshPhysicalNodeMaterial {
  ...
  override copy(source: this): this {
    THREE.MeshPhysicalMaterial.prototype.copy.call(this as unknown as THREE.MeshPhysicalMaterial, source as unknown as THREE.MeshPhysicalMaterial);
    super.copy(source);
    this.transmittanceNode = source.transmittanceNode;
    return this;
  }
...
function createLeafBase(options: Pick<LeafMaterialOptions, 'name' | 'ior' | 'meanReflectance' | 'meanTransmittance' | 'environment'>): LeafNodeMaterial {
  ...
  material.userData.foliageShader = 'thin-leaf';
...
export function createLeafMaterial(options: LeafMaterialOptions): LeafNodeMaterial {
  const material = createLeafBase(options);
  material.userData.leafInput = 'vertex-optics';
...
export interface TexturedLeafMaterialOptions {
  name: string;
  map: THREE.Texture;
  environment?: THREE.Texture;
  ...
  alphaTest?: number;
  ior?: number;
  roughnessFront?: number;
  roughnessBack?: number;
  transmissionScale?: number;
  meanTransmittance?: number;
}

export function createTexturedLeafMaterial(options: TexturedLeafMaterialOptions): LeafNodeMaterial {
  const ior = options.ior ?? 1.45;
  const material = createLeafBase({ name: options.name, ior, meanReflectance: [1, 1, 1], meanTransmittance: options.meanTransmittance ?? .08, environment: options.environment });
  material.color.set(options.tint ?? '#ffffff');
  material.map = options.map;
  material.alphaTest = options.alphaTest ?? .42;
  material.vertexColors = false;
  material.userData.leafInput = 'texture';
  const sample = texture(options.map, uv());
  const tint = vec3(material.color.r, material.color.g, material.color.b);
  const albedo = (options.vertexTint ? sample.rgb.mul(tint).mul(vertexColor().rgb) : sample.rgb.mul(tint)).clamp(0, 1);
  const surfaceReflectance = ((ior - 1) / (ior + 1)) ** 2;
  material.colorNode = vec4(albedo, sample.a);
  material.transmittanceNode = albedo.mul(options.transmissionScale ?? .4).min(vec3(1 - surfaceReflectance).sub(albedo).max(0));
  material.roughnessNode = mix(float(options.roughnessBack ?? .75), float(options.roughnessFront ?? .4), float(frontFacing));
  return material;
}
