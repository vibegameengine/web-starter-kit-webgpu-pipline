 ...
 */
export function createBakeBvh(scene: THREE.Scene): BakeBvh {
  ...
  if (geometries.length === 0) {
    throw new Error('createBakeBvh: nothing tagged Layer.GiStatic');
  }

  const merged = BufferGeometryUtils.mergeGeometries(geometries);
  if (!merged || !merged.index) {
    throw new Error('createBakeBvh: geometry merge failed');
  }
