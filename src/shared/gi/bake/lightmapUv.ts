...
function chartCentre(chart: ChartRequest): THREE.Vector3 {
  const position = chart.mesh.geometry.getAttribute('position');
  const centre = new THREE.Vector3();
  for (const vertex of chart.vertices) centre.add(_a.fromBufferAttribute(position, vertex));
  if (chart.vertices.length > 0) centre.multiplyScalar(1 / chart.vertices.length);
  return centre.applyMatrix4(chart.mesh.matrixWorld);
}
