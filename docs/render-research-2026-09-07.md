...
### Motion blur как восприятие человека, а не как затвор камеры (собрано 2026-09-08)

Вопрос пользователя: почему игровой motion blur кажется неестественным, и как он должен работать, если моделировать не камеру, а зрение. Литература (психофизика зрения, потом рендеринг):

- Burr, «Motion smear», Nature 284:164–165 (1980); Burr, Ross, Morrone (1986) и Burr & Morgan, «Motion deblurring in human vision», Proc. R. Soc. B 264 (1997). Ранние зрительные механизмы интегрируют около 100 мс, то есть движущийся объект должен был бы смазываться как при выдержке 100 мс, но воспринимаемый смаз много короче: зрительная система его подавляет («deblurring»), а окно интеграции сокращается по мере того, как объект движется. Обзор с этими данными и парадигмой измерения (наблюдатель подбирает длину неподвижной линии под виденный смаз): https://www.sciencedirect.com/science/article/pii/S0042698910004700 ; временной ход «motion streaks» (Apthorp, Alais и др., PLOS One 2011): https://journals.plos.org/plosone/article?id=10.1371%2Fjournal.pone.0028675 ; motion sharpening (Hammett, Georgeson): https://www.sciencedirect.com/science/article/pii/S0042698997004306
- Bedell & Lott, «Suppression of motion-produced smear during smooth pursuit eye movements», Current Biology / Vision Research (1996): при одинаковой скорости изображения на сетчатке смаз, воспринимаемый во время плавного слежения (smooth pursuit), примерно на 35 % короче, чем при фиксации; для стимула 150–200 мс. https://pubmed.ncbi.nlm.nih.gov/8805336/
- Tong, Ramamurthy, Patel, Bedell, «The perception of motion smear during eye and head movements», Vision Research 51 (2011); Tong, Stevenson, Bedell, «Signals of eye-muscle proprioception modulate perceived motion smear», J. Vision 8(14):7 (2008); Tong et al., «Attenuation of perceived motion smear during combined eye and head movements» (PMC1752240). Неподвижный фон, который во время слежения проносится по сетчатке, воспринимается ни движущимся, ни смазанным; подавление смаза асимметрично — сильнее для движения против направления слежения; сигнал идёт и от эфферентной копии, и от проприоцепции глазных мышц. https://www.sciencedirect.com/science/article/pii/S0042698910004700 , https://ncbi.nlm.nih.gov/pmc/articles/PMC1752240
- Bedell, Chung, Patel и др., «Motion deblurring during pursuit tracking improves spatial-interval acuity», Vision Research 79 (2013): подавление смаза при слежении улучшает пространственную остроту для движущихся целей — это функциональный механизм, а не побочный эффект. https://ncbi.nlm.nih.gov/pmc/articles/PMC3637418
- Ross, Morrone, Goldberg, Burr, «Changes in visual perception at the time of saccades», Trends in Neurosciences 24:113–121 (2001): саккадическое подавление — чувствительность (особенно магноцеллюлярного, «моторного» пути) падает от ~50 мс до саккады до ~50 мс после; смаз саккады не виден никогда. https://www.cell.com/trends/neurosciences/abstract/S0166-2236(00)01827-0 , нейронный механизм: https://www.science.org/doi/10.1126/science.1068788
- Geisler, «Motion streaks provide a spatial code for motion direction», Nature 400:65 (1999): остаточный смаз («streak») используется мозгом как код направления движения, то есть небольшой смаз — информативен, а не дефект.
- McKee & Nakayama, «The detection of motion in the peripheral visual field», Vision Research 24:25 (1984): различение скорости (ΔV/V ≈ 6 %) на периферии так же точно, как в фовеа; периферия не «чувствительнее к движению», у неё ниже пространственное разрешение. https://pubmed.ncbi.nlm.nih.gov/6695503/ ; периферия нечувствительна к внезапному кратковременному движению: https://pmc.ncbi.nlm.nih.gov/articles/PMC7433379/
- Krajancich, Kellnhofer, Wetzstein, «A perceptual model for eccentricity-dependent spatio-temporal flicker fusion and its applications to foveated graphics», SIGGRAPH 2021: модель того, какая временная информация невидима при данной пространственной частоте, эксцентриситете и яркости; на периферии допустимо меньше пространственных, но не меньше временных деталей. https://www.computationalimaging.org/publications/cff/ , https://arxiv.org/abs/2104.13514
- Denes, Jindal, Mikhailiuk, Mantiuk, «A perceptual model of motion quality for rendering with adaptive refresh rate and resolution», SIGGRAPH 2020: модель качества движения от скорости объекта, частоты кадров и того, следит ли за ним глаз; ключевое разделение — движение, которое глаз ведёт, воспринимается как размытие удержания кадра (hold-type blur), движение, которое глаз не ведёт, — как дрожание/строб. https://www.cl.cam.ac.uk/~rkm38/pdfs/denes2020_motion_adaptive_rendering.pdf , проект: https://www.cl.cam.ac.uk/research/rainbow/projects/motion_quality_model/
- Hold-type blur дисплея: глаз ведёт объект, кадр стоит 16,7 мс при 60 Гц, изображение объекта за это время проезжает по сетчатке v·16,7 мс — это и есть размытие, которое видит игрок на любом LCD/OLED без строба, даже при мгновенном отклике панели. Метод оценки: https://arxiv.org/pdf/1602.07573 ; BiPMAP (предсказание артефактов движения на дисплеях): https://arxiv.org/pdf/2212.03854 ; https://tftcentral.co.uk/articles/motion_blur
- Mantiuk, Bazyluk, Mantiuk, «Gaze-driven object tracking for real time rendering», Computer Graphics Forum 32(2) (2013): с айтрекером — вероятностное определение объекта, который ведёт глаз, по согласованности траектории взгляда с положением и скоростью объекта (скрытая марковская модель); применения — gaze-contingent DoF и резкость ведомого объекта. https://www.cl.cam.ac.uk/~rkm38/pdfs/mantiuk13gdot.pdf ; gaze-dependent DoF: https://link.springer.com/chapter/10.1007/978-3-642-23834-5_1
- Куда смотрит человек при собственном движении (без айтрекера это единственная опора): взгляд собирается у фокуса расширения оптического потока и у путевых точек в 1–2 с впереди, при поворотах — компенсаторное противовращение глаз (то есть глаз ведёт точку сцены, а не экран): Tuhkanen et al., «Humans use predictive gaze strategies to target waypoints for steering», Sci. Rep. 2019: https://www.nature.com/articles/s41598-019-44723-0 ; оптокинетическое слежение за путевыми точками: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7060325/ ; «expansion bias» взгляда к фокусу расширения: https://www.nature.com/articles/s41598-022-15730-5

Что из этого следует для прохода (вывод, не измерение):

1. Смаз — свойство изображения на сетчатке, а не экрана. Скорость, которую надо размывать, — это скорость пикселя относительно точки, которую ведёт глаз, а не относительно камеры: `v_retina = v_pixel − g · v_gaze`, где `g ≈ 0.9` — усиление плавного слежения. Ведомый объект резок; фон и периферия при панораме смазаны; при чистом движении вперёд смазаны края кадра (большая угловая скорость), центр (фокус расширения) — нет. Это в точности «размывать края, а не середину».
2. Ведомый объект уже размыт самим дисплеем (hold-type blur, v·16,7 мс), поэтому рендерить на нём смаз — считать его дважды; это и есть «неестественность» игрового блюра при слежении. Смаз рендерится только для движения, которое глаз не ведёт, — там он заменяет строб на непрерывное движение (Denes 2020).
3. Длина смаза — не 180° затвора (8 мс при 60 Гц), а окно интеграции зрения (~100 мс, Burr) за вычетом подавления (Bedell & Lott: −35 % при слежении; при фиксации смаз ещё короче ожидаемого). Рабочая оценка эффективного окна: 20–40 мс, параметр `integration`, а не `shutter`.
4. Саккада и монтажная склейка — нулевой смаз (саккадическое подавление). Уже сделано: детектор cut в `trackCamera` закрывает затвор.
5. Периферия не нуждается в большем размытии «за низкое разрешение» — там ниже пространственная острота, но временная чувствительность не ниже (Krajancich 2021). Дополнительный радиальный blur «под фовеацию» на мониторе неверен: глаз свободно ходит по экрану. Единственная законная фовеация — через оценку точки взгляда (п. 1).
6. Без айтрекера точку взгляда приходится оценивать: центр кадра / фокус расширения при движении камеры, объект под прицелом или персонаж в TPS, «ведомая» скорость = доминирующая скорость центральной области буфера скоростей со сглаживанием по времени (задержка слежения 100–150 мс). Это гипотеза о взгляде и должна быть выбором в GUI (`gaze: centre | object | camera`), а не молчаливым правилом.
