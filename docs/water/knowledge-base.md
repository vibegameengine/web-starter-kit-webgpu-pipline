...
## 6. Решатель вне кадра (этап 2c, реализовано)

Измерено 2026-09-08, headed Chrome, 4K, `scripts/check-water-sim.mjs`: решатель
Kurganov–Petrova давал **25 quad-рендеров 384×384 в каждом кадре главного потока** —
12 + 12 стадий SSP-RK2 и один view-проход — на **0.37 мс GPU и 1.3–1.6 мс CPU**
главного потока при бюджете кадра 8.3 мс. Дорого именно CPU: это кодирование команд,
и никакое расписание его не убирает, пока проходы кодирует рендерер кадра.

Почему не «развязка по кадрам» (шаг решателя раз в N кадров). Она делит стоимость на
N, но не убирает её: кадр, который всё-таки шагает, платит те же 1.3 мс целиком, то
есть постоянный налог превращается в пик 1 раз из N. Для бюджета 8.3 мс это хуже
налога.

Реализация — `src/entities/water/simWorker.ts` (Web Worker со **своим** устройством
WebGPU на `OffscreenCanvas`) + `simHost.ts` (главный поток) + `waterSim.ts`
(интерфейс `WaterSim`, который реализуют оба пути). `?waterSim=main` возвращает
решатель в кадр — это база для сравнения.

**Обмен полями.** В WebGPU нет разделяемых текстур между устройствами, поэтому поле
не может остаться на GPU: воркер копирует view (depth, u, v, foam) — 384×384 RGBA16F,
1 179 648 Б, шаг строки 3072 Б уже кратен 256 — в mapped-буфер, передаёт `Uint16Array`
как transferable, главный поток подставляет его в `DataTexture` (обмен ссылкой, не
копирование) и помечает `needsUpdate`. Буферы ходят по кругу в обе стороны, так что
после первых двух кадров обмен ничего не аллоцирует. Поле отстаёт на 1–2 кадра: при
периоде зыби 3.2 с это невидимо.

**Насыщение песка.** Единственный вход решателя из главного потока — насыщенность
песка (канал G поля пены 1024², его читает сток инфильтрации по Дарси). Гонять 8 МБ
обратно каждый кадр дороже, чем стоил сам решатель, а этот канал —
чистая функция глубины самого решателя: `max(standing, размытое предыдущее ·
exp(−dt/28))`. Воркер считает его сам на своей сетке 384², радиус размытия оставлен
в uv (0.6/1024), чтобы он расплывался на те же метры.

**Не ждать fence в такте.** `await mapAsync` внутри такта дал один map на 3530 мс,
пока главное устройство занимало GPU, и решатель падал до ~3 тактов/с (вода шла в 40
раз медленнее). Задержка fence — это задержка главного устройства, а не решателя,
поэтому копия отправляется и подхватывается коллбэком, а такт идёт дальше. После
этого — устойчивые 60 Гц, `rate` 0.999–1.001, 1.0–1.6 мс на такт.

**Что это дало и чего не дало.** Из кадра ушли 25 проходов, их 0.37 мс GPU и 1.3–1.6
мс CPU. Суммарное GPU кадра при этом *не* падает на эту величину: тот же GPU по-
прежнему считает решатель, только с другого устройства. Больше того, `gpuMs` (сумма
timestamp-запросов главного рендерера) — нечестная мера, когда устройств два: шесть
переключений воркера в одной сессии дали на 720p 4.49/3.65/3.62 мс с воркером против
4.57/4.56/4.48 без него (кадр *дешевле* с дополнительной нагрузкой — поднимаются
частоты GPU), а на 4K 29.8/30.0/30.1 против 21.4/28.4/28.2 (чередование растягивает
timestamp'ы проходов главного устройства). Настенное время кадра за те же
переключения не сдвинулось: 29.7/32.4/27.5 мс против 30.7/30.0/30.2 мс. Поэтому
`check-water-sim.mjs` сравнивает настенное время кадра с той же сессией при
остановленном воркере, а не суммы GPU двух запусков.

Открыто: первые ~7 с после загрузки воркер голодает (0.1–0.3× реального времени),
пока главный поток доделывает свою загрузку; решатель по-прежнему делает 25
отдельных submit'ов на такт (compute-ядро с несколькими подшагами в одном submit'е —
следующий шаг); `?still=1` останавливает воркер только на старте.
