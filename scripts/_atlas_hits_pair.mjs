...
const variant = process.argv[3] ?? '&atlasHits=0';
...
const off1 = await shot(variant, 'variant-1');
const off2 = await shot(variant, 'variant-2');
const on = await shot('', 'default');
console.log(`default vs '${variant}': ${diff(on, off1).toFixed(3)}/255, run-to-run drift of the variant ${diff(off1, off2).toFixed(3)}/255`);
await browser.close();
