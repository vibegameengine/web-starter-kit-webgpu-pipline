...
const shot = async (every, panning, name) => {
  ...
  await page.goto(`http://127.0.0.1:5188/?scene=beach&hud=0&freezeAt=0&still=1&cam=rocks&grain=0&exposure=1&waterSim=0&contactEvery=${every}`);
  ...
  const png = await page.screenshot(); await writeFile(`shots/contact/${name}.png`, png); await page.close();
  return PNG.sync.read(png);
};
const water = [400, 300, 900, 500];
