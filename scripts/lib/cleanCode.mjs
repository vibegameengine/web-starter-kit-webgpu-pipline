...
//
// Thresholds and their justification: .claude/skills/clean-code.
...
/** Dependencies, build output, vendored code, and the skills, whose examples are meant to be bad. */
const EXCLUDED = /(^|[\\/])(node_modules|dist|coverage|vendor|agents|references|\.claude[\\/]skills)([\\/]|$)/i
...
/**
 * The files these rules apply to, from git, with no pathspec surprises. Untracked
 * files count too: this tree is shared by several sessions whose work is often
 * not yet committed, and a file that exists on disk is debt already agreed to,
 * not a new file starting over a limit.
 */
export function trackedSources(execSync) {
  return execSync('git ls-files --cached --others --exclude-standard', { encoding: 'utf8', maxBuffer: 64e6 })
    .split('\n')
    .filter(Boolean)
    .filter(isSource)
}
