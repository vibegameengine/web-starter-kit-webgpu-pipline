...
const paired = widest !== undefined;
const gapLets = paired && added(widest) >= reference * 0.0005;
const monotone = paired && opened.every((run, i) => i === 0 || added(run) >= added(opened[i - 1]) - reference * 0.0002);
...
const paired_verdict = (ok) => (paired ? (ok ? 'PASS' : 'FAIL') : 'SKIPPED, no gap in this run');
console.log(`sealed room dark: ${sealedDark ? 'PASS' : 'FAIL'}; the widest gap still lets light in: ${paired_verdict(gapLets)}; light grows with the gap: ${paired_verdict(monotone)}`);
if (!paired) console.log('a run without a gap proves only half of A1/A2: pass more than one gap to exercise the pair');
process.exit(sealedDark && (paired ? gapLets && monotone : true) && errors.length === 0 ? 0 : 1);
