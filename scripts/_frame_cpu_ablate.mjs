...
const steps = [
  ['baseline', 'null'],
  ['shadow map cached', 'window.__shadowAutoUpdate(false)'],
  ['shadow map live again', 'window.__shadowAutoUpdate(true)'],
];
for (const [name, action] of steps) {
  await page.evaluate(action);
  await page.evaluate(() => new Promise((resolve) => { let n = 0; const tick = () => (++n > 60 ? resolve() : requestAnimationFrame(tick)); requestAnimationFrame(tick); }));
  console.log(name, JSON.stringify(await page.evaluate(() => window.__frameCpu(240))));
}
clearTimeout(timer);
await browser.close();
